...
| [git-plan.md](git-plan.md) | Git in the IDE: **stages 1 to 3 built** — status in the tree, the branch in the bar, the Changes window with the side-by-side diff, and stage/unstage/discard/commit per file. Why the engine is the CLI and not a binding, why the porcelain and `-z` (and the `Exec.Wait` bug that decision found), and what is left: the log, branches, and the remotes |
