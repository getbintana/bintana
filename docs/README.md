...
| [git-plan.md](git-plan.md) | **Not a feature**: the design for git integration in the IDE — IDE plus one helper and no C, the git CLI over `Exec`, and a side-by-side diff before committing as the core it is built around, with locked scrolling filed as the one gap it leaves open |
