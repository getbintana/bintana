...
is beside it, so it can be copied anywhere and built.

**What it refuses is the point.** A project with no `id`, no metainfo, no icon,
or a metainfo whose `<id>`/`<name>` disagrees with `project.json` stops here
with a sentence. Each of those becomes, half a build later and by another
...
| `Metainfo.create(project, config)` | a minimal metainfo, written beside `project.json` |
| `Metainfo.problems(doc, path, config)` | what does not agree with `project.json` -- the id, the primary name, the file's own name |
| `Metainfo.rename(project, newId)` | moves the file with the id and rewrites `<id>`, `<launchable>` and `<icon>` |
