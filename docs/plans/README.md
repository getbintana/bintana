...
| [task-plan.md](task-plan.md) | **Phase 1 building**: a class that runs in a thread of its own. A first attempt was abandoned for giving the worker a decimal of its own -- and the premise under it turned out to be false, since `JS_NewClass1` registers a main-runtime class id verbatim in a second runtime, so a worker runs the real `init` functions with the real classes and `rad.js` entire. **The frontier is callbacks and not writes**, measured: every mutable process-global in a worker's modules is five variables and the rule already takes all five. Writes are refused in phase 1 as a *deadline and not a doctrine* -- phase 2 is `Lock`, named rather than held because the two runtimes share no heap |
