...
```sh
./tests/run.sh                          # all six projects, 7067 assertions
...
| `tests/report` | `lib/report`: pagination, the group ladder, the totals, the masthead, the bands that grow, the PDF, printing it with no dialog, and the five regressions the library shipped once, and a margin side that is not a finite number refused |
...
`designer` **365**, against **2568** for the whole project — measured at 3.7 s,
...
against 4046 in about eight. It can select because these tests are
