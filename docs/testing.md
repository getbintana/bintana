...
```sh
./tests/run.sh                          # all five projects, 6418 assertions
...
`tests/widgets` is 148 tests listed in `TESTS`, and a filter there **selects** rather
than running a prefix — `./tests/run.sh widgets record` is 71 assertions
in a fifth of a second against 3690 in about eight. It can select because these tests are
