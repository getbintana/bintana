...
```sh
./tests/run.sh                          # all five projects, 6270 assertions
...
`tests/widgets` is 135 tests listed in `TESTS`, and a filter there **selects** rather
than running a prefix — `./tests/run.sh widgets record` is 71 assertions
in a fifth of a second against 3524 in about eight. It can select because these tests are
