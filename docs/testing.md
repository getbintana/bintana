...
```sh
./tests/run.sh                          # all five projects, 6190 assertions
...
| `tests/ide` | The IDE itself, driven the way a user drives it — including **F1 and the reference window** (`help`): which page each of the three answers picks, that the window lands on the member, that a link between two pages is followed and that Back goes back — and **installing the project as a user application** (`apps`): the menu item, the dialog, the id a name slugs to, an entry that points at this executable and this project, a rename that moves the file instead of leaving two, and an uninstall that leaves nothing. The runner points `XDG_DATA_HOME` at a scratch directory, so the entry is not one anybody's menu will offer |
...
`designer` **364**, against **2418** for the whole project — measured at 3.7 s,
8.6 s and 185 s on the machine this was last run on, where what carries over to
...
A partial run says so — `ide: 364 passed, 0 failed  [only welcome, files,
...
in a fifth of a second against 3413 in about three. It can select because these tests are
...
is the same bargain `tests/icons` and `tests/styles` make. 263 widget members and
35 events as this is written, plus 194 on the globals and 58 published by
`lib/` -- the four numbers `./tests/api.sh` prints, and all four were stale
until `Http.Stream` made somebody read them.
