...
```sh
./tests/run.sh                          # all five projects, 6734 assertions
...
`tests/widgets` is 161 tests listed in `TESTS`, and a filter there **selects** rather
than running a prefix — `./tests/run.sh widgets record` is 71 assertions
in a fifth of a second against 3926 in about eight. It can select because these tests are
