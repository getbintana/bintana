...
```sh
./tests/run.sh                          # all five projects, 5920 assertions
...
| `tests/ide` | The IDE itself, driven the way a user drives it — including **F1 and the reference window** (`help`): which page each of the three answers picks, that the window lands on the member, that a link between two pages is followed and that Back goes back |
