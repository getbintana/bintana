...
class Chart extends Component {
     ...
     * the frame it first draws, or on the first time a pointer crosses it, is
     * an attempt to add a name to a sealed object and throws there.
     *
     * **Without values**: what each one means while it is unset is written in
     * the getter that reads it (`this._type || "Bar"`, `this._grid === undefined
     * ? true : this._grid`), and a default said twice is one that will
    ...
    hover; pointer; pressed; dragging; dragged; resetting;
    ...
    set Series(v) {
        ...
        this._series = v.map((s, i) => ({
            ...
            Values: (s.Values || []).map(Chart.numberOf),
    ...
    set YMin(v) { this._ymin = Chart.pinOf("YMin", v); this.Refresh(); }

    get YMax() { return this._ymax === undefined ? "" : this._ymax; }
    set YMax(v) { this._ymax = Chart.pinOf("YMax", v); this.Refresh(); }
    ...
    Canvas_MouseWheel(dx, dy) {
        ...
        /* **A notch that moved nothing is not consumed**, and raises no `Range`:
         * zooming out a view that is already the whole series is the ordinary
         * case at the end of a scroll, and answering `true` there stole the
         * notch from the `Scroller` around the chart. */
        const now = this.view();
