...
/* The widest word in a cell, measured on the words themselves rather than on
 * the markup -- a column cannot be narrower than this without hyphenating
 * somebody's heading, which is what `Headin-gs` in a table looks like. */
function longest(text, font) {
    let widest = 0;
    for (const word of strip(text).split(/\s+/))
        if (word) widest = Math.max(widest, Text.Width(word, font));
    return widest;
}
...
/* The paper a document is exported onto, in points (72 to the inch) -- the same
 * three `lib/report` knows, because a document leaving an application on paper
 * is the same question whichever library drew it. */
const PAPERS = {
...
class Markdown extends Component {
    ...
    set Paper(v) {
        if (!PAPERS[String(v)])
            throw new Error(`Paper: '${v}' is not one of ${Dictionary.Keys(PAPERS).join(", ")}`);
    ...
    SavePdf(path, paper) {
        ...
        if (!PAPERS[name])
            throw new Error(`SavePdf: '${name}' is not one of ${Dictionary.Keys(PAPERS).join(", ")}`);

        const sheet = PAPERS[name];
        const m     = this.margins();

        this.measure(sheet.Width, false);        /* paper is white: the light ink */

        const pages = this.pageBreaks(sheet.Height, m);

        try {
            this.Canvas.SavePdf(path, sheet.Width, sheet.Height, pages.length,
                                (page) => { this._paint = pages[page - 1]; });
        } finally {
            this._paint = null;
            this._measured = null;
        }
    ...
    table(ctx, b, x, w, y) {
        ...
        const need = b.Head.map((c, i) => {
            let word = longest(c, head);
            for (const row of b.Rows) word = Math.max(word, longest(row[i], ctx.body));
        ...
        let bottom = row(heads, (i) => cells.Head[i], head, y, true);
        ...
        for (let n = 0; n < rows.length; n++) {
            ...
            ctx.items.push({ Kind: "Rule", X: x, Y: bottom,
                             ...
                             Color: ink.Rule });
            bottom = row(r, (i) => cells.Rows[n][i], ctx.body, bottom + 1, false);
        }
        return bottom;
    }
