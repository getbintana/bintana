...
function blocks(lines) {
    ...
    while (i < lines.length) {
        ...
        /* **And the delimiter has as many cells as the header**, which is
         * GFM's rule: `a | b` over `---` is a heading underlined, not a table
         * of two columns whose delimiter declared one. */
        if (line.includes("|") && i + 1 < lines.length && RE_DELIM.test(lines[i + 1]) &&
            cells(lines[i]).length > 1 &&
            cells(lines[i + 1]).length === cells(lines[i]).length) {
...
/* A tab in the indentation is spaces to the next multiple of four, which is
 * what CommonMark counts it as. Counted as one column, `\t- nested` under
 * `-\tone` was a sibling rather than a child. Only the leading run and the gap
 * after a list marker are expanded: a tab inside a code line or a sentence is
...
/* Every line but a fenced block's: inside a fence a tab is code, and a
 * Makefile in a document has to come out with its tabs. */
function untabbed(lines) {
    let fence = "";
    return lines.map((line) => {
        if (fence) {
            if (closes(line, fence)) fence = "";
            return line;
        }
        const open = RE_FENCE.exec(line);
        if (open) { fence = open[1][0]; return line; }
        return untab(line);
    });
}
 ...
 *
 * **A letter is a letter of any script.** `\w` is ASCII, so `Введение` came
 * out as an empty anchor and `Instalación` as `instalacin`; `\p{L}\p{M}\p{N}`
 * under `u` keeps them, and an ASCII heading slugs exactly as it always did,
 * since what `\w` kept is still kept and what it dropped is still dropped. */
function slug(text) {
...
/* Two headings with the same words get two anchors, GitHub's way: the second
 * `setup` is `setup-1`, the third `setup-2`. Without it `ScrollTo` and a
 * `#setup` link could only ever reach the first. Walked over the whole tree,
 * because a heading inside a quote or a list item is a heading too. */
function uniqueIds(list, seen) {
    for (const b of list) {
        if (b.Kind === "Heading") {
            let id = b.Id, n = 0;
            ...
            uniqueIds(b.Blocks, seen);
        } else if (b.Kind === "List") {
            for (const item of b.Items) uniqueIds(item, seen);
        }
    }
    return list;
}
...
function inline(text, ink) {
    ...
    while (i < text.length) {
        ...
        if (ch === "*" || ch === "_" || ch === "~") {
            ...
            const boundary = ch !== "_" || (i === 0 || !/[\p{L}\p{N}_]/u.test(text[i - 1]));
...
function findClose(text, from, want, ch) {
    for (let i = from; i < text.length; i++) {
        ...
        /* An underscore does not close inside a word either: `_foo_bar` is
         * literal, as CommonMark has it -- the mirror of the opening rule. And
         * one that could *open* is where the search gives up, so
         * `_foo_bar and _real_` leaves the first literal and emphasises the
         * second rather than spanning from one to the other. */
        if (ch === "_" && /[\p{L}\p{N}]/u.test(text[i + run] || " ")) {
            if (!/[\p{L}\p{N}_]/u.test(text[i - 1] || " ")) return -1;
            i += run - 1;
            continue;
        }
...
class Markdown extends Component {
     ...
     * extensible the moment it is built (`docs/plans/strict-plan.md`), so a
     * field this class only creates when something first *happens* to it -- the
    ...
    set Text(v) {
        ...
        this._blocks = uniqueIds(blocks(untabbed(this._text.replace(/\r\n?/g, "\n")
                                                              .split("\n"))), new Set());
        this._top    = 0;
        this._sel    = null;
        this._needle = "";
    ...
    set Margins(v) {
        ...
        if (v && typeof v === "object" && !Array.isArray(v)) {
            ...
            for (const side of ["Top", "Right", "Bottom", "Left"]) {
                const n = v[side] === undefined ? 24 : v[side];
    ...
    set Scroll(v) { this.place(v); }

    /* **The setter moves and says nothing; the reader's gestures and the verbs
     * say so.** A property setter must not raise an event: a `.form` that
     * declares `Scroll` would raise it while the host's other controls do not
     * exist yet (see AGENTS.md). So `Scroll(y)` is the wheel, a key, the
     * indicator, `ScrollTo` and a find -- everything that goes through
     * `moveTo` -- and an assignment is the program's own and needs no telling. */
    place(v) {
        const n = Math.max(0, Math.min(this.ScrollMax, Math.round(Number(v) || 0)));
        if (n === (this._top || 0)) return false;
        this._top = n;
        this.redraw();
        return true;
    }

    moveTo(v) { if (this.place(v)) this.Emit("Scroll", this._top); }
    ...
    ScrollTo(id) {
        ...
        if (!hit) return false;
        this.moveTo(Math.max(0, hit.Y - this.margins().Top));
        return true;
    }
    ...
    reveal(it, from, to) {
        ...
        this.moveTo(Math.max(0, y - Math.round(view / 4)));
    }
     ...
     * say the job. **Async**, like every dialog in this runtime: `cb` is handed
     * what the dialog settled, and is not called at all when it was cancelled.
     * **To a file it is `SavePdf`.**
     *
     * **`Paper` is what the dialog opens on, and the dialog may overrule it.**
     * It used to be the last word -- the document was paginated against the
     * sheet this was told about, and a size chosen in the dialog scaled the
     * pages instead of re-flowing them, which silently dropped whatever did not
     * fit in the count that had been declared. `Canvas_Paginate` is the answer
     * to that: `Printer` asks the count again once the paper is settled, so the
     * document re-flows for the paper that is really coming out. What `Paper`
     * still decides is the count this hands over to begin with.
     */
    Send(setup, cb) {
        ...
        this.paginate(Printer.Papers[name]);
        ...
        /*
         * **The tidying is in the callback and not in a `finally`.** It was in
         * one, which was right while this call did the printing; now the call
         * returns with the dialog still open, so a `finally` would drop the
         * pagination before a single sheet had been drawn.
         *
         * A cancelled dialog never gets here and leaves the paper's measure in
         * place -- which costs nothing, because the next frame on screen is a
         * different width and `ensure` re-measures for it, the same way it does
         * after any export.
         */
        Printer.Send(this.Canvas, opts, (sent) => {
            this._sheets   = null;
            this._paint    = null;
            this._measured = null;
            fn(sent);
    ...
    /*
     * **Margins that leave no room are refused here**, because this is the
     * first place that knows the paper: `Margins = 421` is a fine gutter on a
     * screen and nothing at all on A4, whose 842 points it covers exactly. With
     * no band between them `end` never passed `from` and the loop below never
     * advanced -- `SavePdf` and `Send` hung for good. With the band positive,
     * every turn moves `from` by at least one cut, so the loop ends; the check
     * inside it says so where it would stop being true.
     */
    pageBreaks(paperHeight, m) {
        ...
        if (!(paperHeight - m.Top - m.Bottom > 0) || !(paperHeight - m.Bottom > 0))
            throw new Error(`Margins: Top ${m.Top} and Bottom ${m.Bottom} leave no room ` +
                            `on a page ${paperHeight} points tall`);
        ...
        while (from < this._height) {
            ...
            if (!(cut > from))
                throw new Error("pageBreaks: a page that does not advance");
    ...
    Canvas_MouseWheel(dx, dy) {
        ...
        this.moveTo(this.Scroll + dy * (this._lh || 16) * 3);
    ...
    Canvas_MouseDown(x, y, button) {
        ...
        if (t && x >= t.X) {
            this.moveTo(Math.round(this.ScrollMax * y / this.viewHeight()));
            return;
        }
    ...
    Canvas_MouseMove(x, y) {
        if (this._drag) {
            ...
            this.moveTo(this._drag.From + (y - this._drag.At) * this.ScrollMax / travel);
            return;
        }
    ...
    Canvas_KeyPress(key, ctrl) {
        ...
        switch (key) {
        case "Down":                    this.moveTo(this.Scroll + line); return true;
        case "Up":                      this.moveTo(this.Scroll - line); return true;
        case "Page_Down": case "space": this.moveTo(this.Scroll + page); return true;
        case "Page_Up":                 this.moveTo(this.Scroll - page); return true;
        case "Home":                    this.moveTo(0); return true;
        case "End":                     this.moveTo(this.ScrollMax); return true;
