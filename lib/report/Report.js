...
/* The paper sizes this library knows, in points (72 to the inch). */
const PAPERS = {
...
class Report extends Component {
     ...
     *
     * The one line the report's own need is: the dialog answers the printer,
     * the paper, the copies and the range, GTK's preview shows what is about
     * to come out, and then the same `Draw` runs once per page -- the pages
     * `SavePdf` already writes. Paper and orientation start where this report
     * has them; `{ Copies, From, To, ToFile }` say the rest, and the answer
     * is what the dialog settled (`{ Copies, From, To }`, or `null` when it
     * was cancelled). `ToFile` writes a PDF with no dialog, which is both
     * "print to PDF" and the road a test can take.
     */
    Print(options) {
        const o = options === undefined || options === null ? {} : options;
        if (typeof o !== "object" || Array.isArray(o))
            throw new Error("Print expects an options object");
        ...
        if (o.ToFile !== undefined) opts.ToFile = o.ToFile;

        try {
            return this.Canvas.Print(opts, (page) => { this._saving = page; });
        } finally {
            this._saving = undefined;
        }
    }
     ...
     */
    paperSize() {
        const p = PAPERS[this.Paper] || PAPERS.A4;
