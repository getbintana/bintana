...
class Report extends Component {
     ...
     * `--strict` (`docs/plans/strict-plan.md`): a field this class only creates
     * the first time it paginates is an attempt to add a name to a sealed
     * object, and `PageCount` on a strict run threw before this block existed.
     *
    ...
    remeasure() {
        this.measure();
        this._page = Math.max(1, Math.min(this._count || 1, this._page || 1));
        this.redraw();
    }
    ...
    measure() {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            if (prev === null) {
                ...
                let d = 0;
                while (d < groups.length && Report.sameKey(key[d], prev[d])) d++;
    ...
    computeTotals(rows, fields) {
        ...
        for (const f of fields) {
            ...
            const less = Report.orderFor(present);
